...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!summary) {
      return NextResponse.json({ success: false, error: 'Summary data is required' }, { status: 400 });
    }
    ...
    if (summary.profitMargin > 20) {
      advice += "Your profit margins are looking great (above 20%). Consider reinvesting some of this profit into marketing or expanding your inventory with new product lines.\n";
    } else {
      advice += "Your profit margins are relatively low. Consider analyzing your top-selling products to see if you can slightly increase prices, or negotiate better buying prices from your suppliers.\n";
    }

    if (summary.totalSalesCount < 10) {
      advice += "Sales volume is a bit low today. You might want to run a local promotion or engage with recurring customers.\n";
    } else {
      advice += "Great sales volume! Keep up the good work and ensure your best-selling items remain well-stocked.\n";
    }
