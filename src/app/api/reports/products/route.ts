...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const productDetails = await prisma.product.findMany({
      ...
      select: { id: true, name: true, nameBn: true, buyingPrice: true, unit: true },
    });
    ...
    return NextResponse.json({ error: 'Failed to fetch product report', details: (error instanceof Error ? error.message : String(error)) }, { status: 500 });
  }
}
